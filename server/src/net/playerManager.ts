...
import { Player, Position, Velocity, Renderable, Stats } from '@shared/ecs/components'
import { PlayerJoinMessage, PlayerMoveMessage, PlayerUpdateMessage } from '@shared/messages'
...
export class PlayerManager {
  ...
  private setupMessageHandlers() {
    ...
    this.server.onMessage('PLAYER_MOVE', (client, message: PlayerMoveMessage) => {
      this.handlePlayerMove(client, message)
    })
  }

  // Create a player entity when a client connects
  createPlayer(client: ConnectedClient, playerData: { name: string }): EntityId {
  ...
  private handlePlayerMove(client: ConnectedClient, message: PlayerMoveMessage) {
  ...
  updatePlayer(playerId: string, updates: Partial<{
    ...
    if (updates.spriteId !== undefined || updates.frame !== undefined) {
      const renderable = this.world.getComponent(entityId, 'renderable')
      if (renderable) {
        if (updates.spriteId !== undefined) renderable.spriteId = updates.spriteId
